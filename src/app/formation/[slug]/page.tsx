...
export default function FormationDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAFAF8] flex flex-col items-center justify-center p-6">
        ...
        <div className="h-6 bg-[#E8E8E5] rounded w-64 mb-4 animate-pulse" />
        <div className="h-4 bg-[#E8E8E5] rounded w-48 animate-pulse" />
      </div>
    );
  }
  ...
  if (error || !content) {
    return (
      <div className="min-h-screen bg-[#FAFAF8] flex flex-col items-center justify-center p-6 text-center">
        <Info className="size-12 text-[#A3A3A3] mb-4" />
        <h1 className="text-xl font-bold text-[#1A1916] mb-2">Contenu introuvable</h1>
        <p className="text-[#6B6B6B] mb-6">Ce tutoriel n'existe pas ou n'est plus disponible.</p>
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8] text-[#1A1916] font-sans selection:bg-[#2A9D8F]/20 pb-20">
      <Head>
      ...
      <header className="sticky top-0 z-20 border-b border-[#E8E8E5] bg-white/80 backdrop-blur-md px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/formation" className="p-2 -ml-2 rounded-lg text-[#6B6B6B] hover:bg-[#FAFAF8] transition-colors">
            <ArrowLeft className="size-5" />
          </Link>
          <div className="h-5 w-px bg-[#E8E8E5]" />
          <span className="font-medium text-[#1A1916] truncate max-w-[200px] sm:max-w-none">{content.title}</span>
        </div>
      </header>
      ...
      <main className="max-w-[840px] mx-auto px-6 py-12 space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(duration || content.reading_time_minutes) && (
              <span className="text-[12px] font-medium text-[#6B6B6B] bg-white border border-[#E8E8E5] px-2.5 py-1 rounded-md flex items-center gap-1.5 shadow-[0_1px_2px_rgba(26,25,22,0.02)]">
          ...
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-[#1A1916] leading-tight">
            {content.title}
          </h1>
        ...
        {content.type === "video" && content.video_variants && content.video_variants.length > 0 && (
          <div className="bg-white border border-[#E8E8E5] rounded-xl p-4 shadow-sm flex flex-col sm:flex-row sm:items-center gap-4">
            <span className="text-sm font-semibold text-[#1A1916]">Voir la version :</span>
            <div className="flex flex-wrap gap-2">
              {content.video_variants.sort((a, b) => a.order - b.order).map((variant) => {
                const Icon = variant.device === "desktop" ? Monitor : variant.device === "tablet" ? Tablet : Smartphone;
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                      isActive
                        ? "bg-[#1A1916] text-white shadow-md scale-105"
                        : "bg-[#FAFAF8] text-[#6B6B6B] border border-[#E8E8E5] hover:bg-white hover:border-[#2A9D8F]/50"
                    }`}
                  >
                    <Icon className="size-4" />
                    <span className="capitalize">{variant.device === "desktop" ? "PC" : variant.device === "tablet" ? "Tablette" : "Mobile"}</span>
        ...
        {content.type === "video" && videoUrl && (
          <div className="rounded-2xl border border-[#E8E8E5] overflow-hidden bg-black shadow-[0_4px_24px_rgba(26,25,22,0.04)]">
            <video 
        ...
        {content.content && (
          <div className="bg-white rounded-2xl border border-[#E8E8E5] p-6 md:p-10 shadow-[0_2px_12px_rgba(26,25,22,0.02)] prose prose-zinc max-w-none prose-headings:font-semibold prose-headings:text-[#1A1916] prose-a:text-[#2A9D8F] prose-a:no-underline hover:prose-a:underline prose-p:text-[#1A1916] prose-p:leading-relaxed prose-li:text-[#1A1916]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {content.attachments.map((file, i) => (
                <a
                  ...
                  className="flex items-center gap-3 p-4 bg-white rounded-xl border border-[#E8E8E5] hover:border-[#2A9D8F]/50 transition-colors group shadow-sm"
                >
                  <div className="grid size-10 place-items-center rounded-lg bg-[#FAFAF8] text-[#6B6B6B] group-hover:text-[#2A9D8F] group-hover:bg-[#E5F5F3] transition-colors">
                    <FileText className="size-5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-[#1A1916] truncate">{file.name}</p>
                    {file.size && <p className="text-xs text-[#A3A3A3]">{file.size}</p>}
                  </div>
                </a>
              ))}
            </div>
