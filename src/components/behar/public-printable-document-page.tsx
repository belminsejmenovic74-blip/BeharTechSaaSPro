...
import { Copy, Download, FileText, Phone, Printer, ShieldCheck } from "lucide-react";
...
export function PublicPrintableDocumentPage({ token }: { token: string }) {
    ...
    return (
      <div className="grid min-h-screen place-items-center px-6" style={{ background: COLORS.bg, color: COLORS.text }}>
        <div className="w-full max-w-md rounded-[18px] border bg-white p-8 text-center shadow-[0_1px_3px_rgba(26,25,22,0.04)]" style={{ borderColor: COLORS.border }}>
          ...
          <p className="mt-2 text-sm" style={{ color: COLORS.sub }}>Ce document n'est plus public ou le lien a expiré.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen px-4 py-6" style={{ background: COLORS.bg, color: COLORS.text }}>
      <div className="no-print mx-auto mb-4 flex w-full max-w-[860px] flex-wrap items-center justify-between gap-3 rounded-[16px] border bg-white p-3 shadow-[0_1px_3px_rgba(26,25,22,0.04)]" style={{ borderColor: COLORS.border }}>
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[12px]" style={{ color: COLORS.sub }}>{data.document.title}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {publicUrl ? (
            <button type="button" onClick={() => { void navigator.clipboard?.writeText(publicUrl); toast.success("Lien copié"); }} className="grid size-10 place-items-center rounded-[12px] border bg-white" style={{ borderColor: COLORS.border }} aria-label="Copier le lien">
      ...
      <article ref={documentRef} className="print-document print-page mx-auto flex min-h-[1123px] w-full max-w-[860px] flex-col rounded-[16px] border bg-white p-8 shadow-[0_1px_3px_rgba(26,25,22,0.04)] print:min-h-screen print:rounded-none print:border-0 print:p-6 print:shadow-none" style={{ borderColor: COLORS.border }}>
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2.5">
                {data.workshop.logoUrl ? (
                  ...
                  <img src={data.workshop.logoUrl} alt={data.workshop.name} className="h-10 max-w-[160px] object-contain" />
                ) : (
                  <span className="grid size-10 place-items-center rounded-[10px] text-white" style={{ background: COLORS.accent }}><ShieldCheck className="size-5" /></span>
                )}
              ...
              <div className="mt-3 text-[12px] leading-relaxed" style={{ color: COLORS.sub }}>
                ...
                {data.workshop.businessId ? (
                  <p>{data.workshop.country === "CH" ? "IDE / UID" : "SIRET"} : {data.workshop.businessId}</p>
                ) : null}
                {data.workshop.vatApplicable && data.workshop.vatNumber ? (
                  <p>{data.workshop.country === "CH" ? "N° TVA suisse" : "TVA intracommunautaire"} : {data.workshop.vatNumber}</p>
                ) : null}
        ...
        <main className="flex-1 space-y-5 py-6">
          <section className="grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-[14px] border p-4" style={{ borderColor: COLORS.border }}>
              ...
              <dl className="mt-3 space-y-2 text-[12px]">
                ...
                <Row label="Appareil" value={[data.repair?.deviceBrand, data.repair?.deviceModel].filter(Boolean).join(" ") || "-"} />
                <Row label="Intervention" value={data.repair?.interventionLabel || data.repair?.issueDescription || "-"} />
          ...
          <section className="overflow-hidden rounded-[14px] border" style={{ borderColor: COLORS.border }}>
            <div className="grid grid-cols-[1fr_70px_112px_112px] px-4 py-3 font-semibold text-[11px] uppercase tracking-wide" style={{ background: COLORS.soft, color: COLORS.sub }}>
              <span>Désignation</span><span className="text-center">Qté</span><span className="text-right">Prix unitaire</span><span className="text-right">Total</span>
            </div>
            {data.lines.map((line, index) => (
              <div key={`${line.label}-${index}`} className="grid grid-cols-[1fr_70px_112px_112px] border-t px-4 py-4 text-[13px]" style={{ borderColor: COLORS.border }}>
                <span className="font-medium">{line.label}</span>
                <span className="text-center" style={{ color: COLORS.sub }}>{line.quantity}</span>
                <span className="text-right" style={{ color: COLORS.sub }}>{formatMoney(line.unitPriceTtc, data.workshop.currency)}</span>
                <span className="text-right font-semibold">{formatMoney(line.totalTtc, data.workshop.currency)}</span>
              </div>
            ))}
          </section>

          <section className="ml-auto w-full max-w-[340px] rounded-[14px] border p-4" style={{ borderColor: COLORS.border, background: COLORS.soft }}>
          ...
          {data.workshop.vatMention ? (
            <section className="rounded-[14px] border p-4 text-[11px] leading-relaxed" style={{ borderColor: COLORS.border, background: COLORS.soft }}>
              {data.workshop.vatMention}
            </section>
          ) : null}
          ...
          {data.documentType === "repair_intake" ? (
            <section className="rounded-[14px] border p-4 text-[11px] leading-relaxed" style={{ borderColor: COLORS.border, background: COLORS.soft }}>
              <p><strong>Prise en charge.</strong> Le client confie l'appareil à l'atelier pour diagnostic ou intervention. Le client reste responsable de la sauvegarde préalable de ses données.</p>
              <p className="mt-2"><strong>RGPD.</strong> Les informations affichées sont limitées au suivi et au document client. Les notes internes, marges, fournisseurs et prix d'achat ne sont jamais affichés sur cette page.</p>
            </section>
          ) : null}
        </main>

        <footer className="mt-auto border-t pt-5 text-[10px] leading-relaxed" style={{ borderColor: COLORS.border, color: "#6B6B6B" }}>
