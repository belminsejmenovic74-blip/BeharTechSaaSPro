...
export function PublicPrintableDocumentPage({ token }: { token: string }) {
  ...
  return (
    <div className="min-h-screen px-4 py-6" style={{ background: COLORS.bg, color: COLORS.text }}>
      ...
      <article className="print-document print-page mx-auto flex min-h-[1123px] w-full max-w-[860px] flex-col rounded-[16px] border bg-white p-8 shadow-[0_1px_3px_rgba(26,25,22,0.04)] print:min-h-screen print:rounded-none print:border-0 print:p-6 print:shadow-none" style={{ borderColor: COLORS.border }}>
          ...
          <div className="flex items-start justify-between gap-8">
            <div className="min-w-0">
              ...
              <div className="mt-3 text-[12px] leading-relaxed" style={{ color: COLORS.sub }}>
                ...
                {data.workshop.city ? <p>{data.workshop.city}</p> : null}
        ...
        <main className="flex-1 space-y-5 py-6">
          ...
          <section className="overflow-hidden rounded-[14px] border" style={{ borderColor: COLORS.border }}>
            ...
            {data.lines.map((line, index) => (
              <div key={`${line.label}-${index}`} className="grid grid-cols-[1fr_70px_112px_112px] border-t px-4 py-4 text-[13px]" style={{ borderColor: COLORS.border }}>
                ...
                <span className="text-right" style={{ color: COLORS.sub }}>{money(line.unitPriceTtc)}</span>
                <span className="text-right font-semibold">{money(line.totalTtc)}</span>
              </div>
            ))}
          </section>
          ...
          <section className="ml-auto w-full max-w-[340px] rounded-[14px] border p-4" style={{ borderColor: COLORS.border, background: COLORS.soft }}>
            <div className="flex items-center justify-between gap-4 font-bold">
              <span>Total TTC</span>
              <span className="text-[22px]">{money(data.totalTtc)}</span>
            </div>
          </section>
