...
export function ModeAccessGuard({
  ...
  return (
    <main className="grid min-h-svh place-items-center bg-white px-5 py-10 text-[#1A1916]">
      <section className="w-full max-w-[500px] rounded-[20px] border border-[#E8E8E5] bg-white p-7 text-center shadow-[0_10px_40px_rgba(26,25,22,0.06)]">
        <span className="mx-auto grid size-12 place-items-center rounded-full border border-[#E8E8E5] bg-white text-[#6B6B6B]">
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-2">
            {alternatives.map(({ href, label: destinationLabel, icon: Icon }) => (
              <Link
                key={href}
                href={href}
                className="inline-flex h-11 items-center justify-center gap-2 rounded-[11px] bg-[#1A1916] px-4 text-sm font-semibold text-white transition hover:bg-[#2A2925]"
              >
        ...
        <button
          type="button"
          onClick={() => logout()}
          className="mt-3 inline-flex h-10 items-center justify-center gap-2 rounded-[10px] px-4 text-sm font-medium text-[#6B6B6B] transition hover:bg-[#F4FBF9] hover:text-[#1A1916]"
        >
