(() => {
  ...
  function renderSearch(host, index) {
    ...
    root.innerHTML = [
      ...
      ".search{display:grid;grid-template-columns:repeat(3,minmax(0,1fr)) minmax(280px,.92fr);align-items:stretch;width:100%;max-width:1380px;margin:0 auto;border:1px solid #E2E2DF;border-radius:22px;background:#fff;box-shadow:0 16px 44px rgba(20,24,28,.08);overflow:hidden}",
      ".field{min-width:0;padding:20px 24px;border-right:1px solid #E8E8E5;background:#fff}",
      "label{display:block;margin:0 0 7px;color:#858D98;font-size:13px;font-weight:650;line-height:1.25}",
      "select{display:block;width:100%;min-height:38px;border:1px solid transparent;border-radius:10px;background:#fff;color:#152231;font:inherit;font-size:18px;font-weight:700;line-height:1.25;outline:none;cursor:pointer}",
      "select:disabled{cursor:not-allowed;color:#B1B7BE;background:#fff}",
      "select:focus-visible{border-color:#111;box-shadow:0 0 0 3px rgba(17,17,17,.08)}",
      ...
      ".submit{align-self:stretch;margin:10px;border:1px solid #111;border-radius:16px;background:#111;color:#fff;padding:16px 22px;font:inherit;font-size:16px;font-weight:750;line-height:1.25;cursor:pointer;transition:transform .15s ease,background .15s ease,opacity .15s ease}",
      ".submit:hover:not(:disabled){background:#292929;transform:translateY(-1px)}",
      ".submit:focus-visible{outline:3px solid rgba(42,157,143,.32);outline-offset:2px}",
      ".submit:disabled{cursor:not-allowed;border-color:#D8D8D4;background:#F4F4F2;color:#999;opacity:1}",
